...
module.exports = {
    ...
    callback: async ({ sock, m, groupMetadata }) => {
        ...
        if (groupMetadata.announce) sock.groupSettingUpdate(m.chat, "not_announcement")
        m.reply("Open time di mulai dari sekarang")
        setTimeout(() => {
        sock.groupSettingUpdate(m.chat, "announcement")
        m.reply("Success menutup group, Sekarang hamua admin yang bisa mengirim pesan")
        }, Number(timer))
    }
}
